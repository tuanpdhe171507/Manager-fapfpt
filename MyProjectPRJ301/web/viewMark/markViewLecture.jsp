<%-- 
    Document   : AttendanceSV
    Created on : Mar 7, 2024, 7:39:17 PM
    Author     : G5 5590
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>JSP Page</title>
        <style>
            .text1{
                margin-top: 40px;
                margin-left: -60px;

            }
            .rectangle {
                width: 1230px;
                height: 50px;
                background-color: #CCCCCC;
                margin-left: -60px;
                margin-top: 20px;
                margin-bottom: 20px;
                margin-left: 120px;
                border-radius: 5px;
            }

            table {
                width: 100%;
                border-collapse: collapse;
            }
            table, th, td {
                border: 1px solid black;
            }
            th, td {
                padding: 10px;
                text-align: left;
            }
            .cc{
                display: flex;
                justify-content: center;
            }
            .col-md-6 {
                width: 50%;
            }
            .cc{
                width: 100%;
            }
            .text2{
                margin-left: 125px;
                font-size: 30px;
            }
            .text3{
                display: flex;

                font-size: 25px;
            }
            .bang1{
                width: 500px;
                height: 100%;
                margin-left: 140px;
            }

            .text4 {
                vertical-align: top;
            }
            .text5{
                width: 500px;
            }
            .bang2{
                width: 800px;
                height: 100%
            }
            .text6{
                color: green;
            }
            .text7{
                color: red;
            }
            .text10{
                display: flex;

            }
            .text8{
                margin-left: 120px;
            }
            .text9{
                margin-left: 200px;
            }
            th{
                background-color: rgba(0, 0, 255, 0.7);
            }
            .text11{
                background-color: rgba(0, 0, 255, 0.6);
            }
            .text12{
                background-color: rgba(255, 0, 0, 0.7);
            }
            .text13{
                width: 800px;
                height: 30px;
            }

            .text28 {
                display: inline-block;
                vertical-align: top;
            }
            .text88{
                margin-top: 7px;
            }
            .hh{
                margin-top: 40px;
            }
            .additional-line {
                width: 119%;
                height: 1px;
                background-color:#000;
                margin-top: -6px;
                margin-left: -100px;
            }
            .circle-wrapper {
                width: 15px; /* Đường kính của hình tròn */
                height: 15px; /* Đường kính của hình tròn */
                border-radius: 50%; /* Khoanh tròn phần tử */
                border: 1px solid #000;
                display: flex;
                justify-content: center;
                align-items: center;
                margin-top: 5px;
            }
            .text33{
                display: flex;
                justify-content: center;
            }
            .text44{
                color: blue;
            }
            .bangbig{
                display: flex;
            }
            .bang11{

                height: 600px;
            }


            .bang33{
                width: 100%;
                height: 100%;
            }
            .bang{
                width: 100%;
                border-collapse: collapse;
            }

            .text888{
                width: 100px;
            }
            .text999{
                width: 100px;
            }
            .text444{
                height: 50px;
            }
            .row2{
                height: 50px;
            }
            .row4{
                height:  69px;

            }
            .bang1 tr {
                height: 25px !important;
                line-height: 16px;
                vertical-align: middle;

            }
            .text888{
                width: 160px;
            }
            .text55{
                width: 166px;
            }
            .text66{
                width: 166px;
            }
            .text1000{
                width: 167px;
            }
            .text101{
                width: 100px;
            }
            .text111{
                width: 100px;
            }
            .cv{
                width: 100px;
            }
            .ck{
                width: 103px;
            }
            .score{
                display: flex;
            }
            .text200{
                font-size: 25px;
                margin-left: 0px;
                margin-top: 15px;
            }
            .text211{
                font-size: 25px;
                margin-left: 70px;
                margin-top: 15px;
            }
            .text222{
                font-size: 25px;
                margin-left: 170px;
                margin-top: 15px;
            }
            .text233{
                margin-top: 50px;
                margin-left: -45px;
                font-size: 25px;
            }

            .bang11 {
                width: 500px; /* Đặt độ rộng của bảng */
                table-layout: fixed; /* Thiết lập bảng để không tự động điều chỉnh kích thước cột */
            }

            .bang11 td {
                padding: 0px; /* Khoảng cách giữa nội dung và biên của các ô */
                border: 1px solid black; /* Đường viền của ô */
                text-align: left; /* Canh lề văn bản */
                word-wrap: break-word; /* Tự động ngắt dòng khi cần thiết */
            }

            /* Độ rộng cố định cho các cột */
            .bang11 td:nth-child(1), /* Cột 1 */
            .bang11 td:nth-child(2), /* Cột 2 */
            .bang11 td:nth-child(3) { /* Cột 3 */
                width: 150px; /* Độ rộng cố định cho mỗi cột */
            }
            .hoho{
                background-color:  rgba(0, 128, 0, 0.6);
                border: none;
                color: white;
                margin-top: 10px;
            }
            .haha{
                 background-color:  rgba(0, 128, 0, 0.6);
                border: none;
                color: white;
                margin-top: 10px;
                height: 35px;
                width: 57px;
            }
            .hehe{

                display: flex;
                justify-content: right;

            }
            .ngan{
                margin-top: 9px;
            }

        </style>


        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
        <script>
            $(document).ready(function () {

                var rows = $('.bang11 .row2');


                var seenRows = {
                }
                ;


                rows.each(function () {
                    var rowContent = $(this).text().trim();


                    if (seenRows[rowContent]) {
                        $(this).remove();
                    } else {
                        seenRows[rowContent] = true;
                    }
                }
                );
            }
            );

        </script>
        <!--        <script>
                    function sendSemeterId(semeterId) {
                        var form = document.createElement("form");
                        form.setAttribute("method", "GET");
                       
        
                        var hiddenField = document.createElement("input");
                        hiddenField.setAttribute("type", "hidden");
                        hiddenField.setAttribute("name", "semeterId");
                        hiddenField.setAttribute("value", semeterId);
        
                        form.appendChild(hiddenField);
                        document.body.appendChild(form);
                        form.submit();
                    }
                   
        
                </script>-->
        <script>
            function sendSemesterId(semesterId, studentID) {
                var form = document.createElement("form");
                form.setAttribute("method", "GET");
                form.setAttribute("action", "mark"); // Thay thế "your_destination_url_here" bằng URL đích của bạn

                var semesterIdField = document.createElement("input");
                semesterIdField.setAttribute("type", "hidden");
                semesterIdField.setAttribute("name", "semesterId");
                semesterIdField.setAttribute("value", semesterId);

                var studentIdField = document.createElement("input");
                studentIdField.setAttribute("type", "hidden");
                studentIdField.setAttribute("name", "studentID");
                studentIdField.setAttribute("value", studentID);

                form.appendChild(semesterIdField);
                form.appendChild(studentIdField);

                document.body.appendChild(form);
                form.submit();
            }
        </script>

    </head>
    <body>
        <form action="mark" method="GET">

            <div class="container">
                <div class="row cc">
                    <div class="col-md-6">
                        <h1 class="text1">FPT University Academic Portal</h1>
                    </div>
                    <div class="col-md-6">
                        <img src="data:image/jpeg;base64,">

                    </div>
                </div>
            </div>  
            <div class="rectangle">
                <div class="hehe">
                    <a href="login"><input class="haha" value="Logout"/></a><h4 class="ngan">|</h4>
                    <input class="hoho" type="button" value="CAMPUS: FPTU-Hòa Lạc" readonly/>
                </div>

            </div>

            <div class="text2">View Mark for ${s.lastName}${s.fristName} (${s.rollNumber})</div>

            <div class="text3">
                <div class="text8">
                    Select a campus/program, term, course ...
                </div>
                <div class="text9">
                    ... then see report
                </div>
            </div>
            <div class="text10"> 
                <table class="bang1">
                    <tr>
                        <th>CAMPUS/PROGRAM</th>
                        <th>TERM</th>
                        <th class="text5">COURSE</th>
                    </tr>

                    <c:forEach items="${requestScope.listSemeter}" var="i" varStatus="loop">

                        <tr>
                            <c:if test="${loop.index == 0}">
                                <td rowspan="${fn:length(requestScope.listSemeter)}">FU-HL</td>
                            </c:if>
                            <td>
                                <form id="semeterForm_${i.semeterID}" action="mark" method="POST">
                                    <div name="semeterid" onclick="document.getElementById('semeterForm_${i.semeterID}').submit()">
                                        <a href="#" onclick="sendSemeterId('${i.semeterID}')">${i.semeterName}</a>

                                    </div>
                                    <input type="hidden"name="semeterId" value="${i.semeterID}">
<!--                                    <input type="hidden" name="studentID" value="${requestScope.studentID}">-->
                                </form>
                            </td>
                            <c:if test="${loop.index == 0}">
                                <c:set var="groupNameRowCount" value="0" />
                                <c:forEach items="${requestScope.listSubject}" var="s">
                                    <c:set var="groupNameRowCount" value="${groupNameRowCount + 2}" />
                                </c:forEach>
                                <td class="text4" rowspan="${groupNameRowCount}">
                                    <c:forEach items="${requestScope.listSubject}" var="s" varStatus="status">
                                        <form id="semeterForm1ff_${s.subject.subjectID}" action="mark" method="GET">
                                            <div name="semeterid1ff" onclick="document.getElementById('semeterForm1ff_${s.subject.subjectID}').submit()">
                                                <a href="mark?semeterId1ff=${s.subject.subjectID}&studentID=${requestScope.studentID}">${s.subject.subjectName}(${s.subject.code})</a>(${s.groupName}start ${s.semeter.startDate})
                                            </div>
                                            <input type="hidden" name="semeterId1ff" value="${s.subject.subjectID}">

                                        </form>
                                        <br/>
                                        <c:if test="${not status.last}">
                                            <br/>
                                        </c:if>
                                    </c:forEach>
                                </td>
                            </c:if>
                        </tr>
                    </c:forEach>
                </table>


                <c:if test="${listGrade!=null && listGrade.size()>0}">



                    <div>
                        <table class="bang">

                            <tr>
                                <th class="text55 tuan">GRADE CATEGORY</th>
                                <th class="text66 tuan">GRADE ITEM</th>
                                <th class="text1000 tuan">WEIGHT</th>
                                <th class="text101 tuan">VALUE</th>
                                <th class="text111 tuan">COMMENT</th>
                            </tr>



                        </table>

                        <div class="bangbig">
                            <div>
                                <table class="bang11">

                                    <input type="hidden" name="hihi" value="${requestScope.studet}">
                                    <c:forEach items="${requestScope.listGrade}" var="i">
                                        <tr class="row2">
                                            <td class="text888">${i.assessmentName}</td>
                                            <c:choose>
                                                <c:when test="${i.assessmentName eq 'Workshop'}">
                                                    <td>Workshop1</td>
                                                </c:when>
                                                <c:when test="${i.assessmentName eq 'Exercise'}">
                                                    <td>Exercise1</td>
                                                </c:when>
                                                <c:when test="${i.assessmentName eq 'Progress test'}">
                                                    <td>Progress test 1</td>
                                                </c:when>

                                                <c:when test="${i.assessmentName eq 'Assignment' and i.subjectid.subjectID == 22}">
                                                    <td>Assignment 1</td>
                                                </c:when>
                                                <c:when test="${i.assessmentName eq 'Final Exam'  and i.subjectid.subjectID == 23}">
                                                    <c:choose>
                                                        <c:when test="${finalExamChanged}">
                                                            <td>FE: GVR</td>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <td>FE: Listening</td>
                                                            <c:set var="finalExamChanged" value="true" />
                                                        </c:otherwise>
                                                    </c:choose>
                                                </c:when>

                                                <c:when test="${i.assessmentName eq 'Final Exam Resit'  and i.subjectid.subjectID == 23}">
                                                    <c:choose>
                                                        <c:when test="${finalExamChanged12}">


                                                            <td>FE: GVR</td>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <td>FE: Listening</td>                                              
                                                            <c:set var="finalExamChanged12" value="true" />
                                                        </c:otherwise>
                                                    </c:choose>
                                                </c:when>


                                                <c:otherwise>
                                                    <td class="text777">${i.assessmentName}<br>Total</td>
                                                    </c:otherwise>
                                                </c:choose>
                                            <td class="text999">${Math.round(i.weight * 1000) / 10}%<br>Total:${Math.round(i.weight * 1000) / 10}</td>

                                        </tr>




                                        <c:if test="${i.assessmentName eq 'Workshop'}">
                                            <tr class="row2">
                                                <td></td>
                                                <td>Workshop2<br>Total</td>
                                                <td>${Math.round(i.weight * 1000) / 10}%<br>Total:W1+W2=${Math.round(i.weight * 1000) / 10+Math.round(i.weight * 1000) / 10}</td>

                                            </tr>
                                        </c:if>
                                        <c:if test="${i.assessmentName eq 'Exercise'}">
                                            <tr class="row2">
                                                <td></td>
                                                <td>Exercise2<br>Total</td>
                                                <td>${Math.round(i.weight * 1000) / 10}%<br>Total:Ex1+Ex2=${(Math.round(i.weight * 1000) / 10)+(Math.round(i.weight * 1000) / 10)}%</td>

                                            </tr>

                                        </c:if>
                                        <c:if test="${i.assessmentName eq 'Progress test' and i.subjectid.subjectID == 25}">

                                            <tr class="row3 row2 text444">
                                                <td></td>

                                                <td class="">Progress test 2<br>Total</td>
                                                <td>${Math.round(i.weight * 1000) / 10}%<br>Total:P1+P2=${Math.round(i.weight * 1000) / 10+Math.round(i.weight * 1000) / 10}%</td>

                                            </tr>
                                        </c:if>
                                        <c:if test="${i.assessmentName eq 'Progress test' and i.subjectid.subjectID == 23}">
                                            <tr class="row2">
                                                <td></td>
                                                <td>Progress test 2<br>Total</td>
                                                <td>${Math.round(i.weight * 1000) / 10}%<br>Total:P1+P2=${(Math.round(i.weight * 1000) / 10)+(Math.round(i.weight * 1000) / 10)}%</td>

                                            </tr>
                                        </c:if>
                                        <c:if test="${i.assessmentName eq 'Progress test' and i.subjectid.subjectID == 22}">

                                            <tr class="row3 row2 text444">
                                                <td></td>

                                                <td class="">Progress test 2</td>
                                                <td>${Math.round(i.weight * 1000) / 10}%<br>Total:${Math.round(i.weight * 1000) / 10}%</td>

                                            </tr>
                                        </c:if>
                                        <c:if test="${i.assessmentName eq 'Progress test' and i.subjectid.subjectID == 22}">

                                            <tr class="row3 row2 text444">
                                                <td></td>

                                                <td class="">Progress test 3<br>Total</td>
                                                <td class="huong">${Math.round(i.weight * 1000) / 10}%<br>Total:P1+P2+P3=${(Math.round(i.weight * 1000) / 10)+(Math.round(i.weight * 1000) / 10)+(Math.round(i.weight * 1000) / 10)}%</td>

                                            </tr>
                                        </c:if>
                                        <c:if test="${i.assessmentName eq 'Assignment' and i.subjectid.subjectID == 22}">
                                            <tr class="row2">
                                                <td></td>
                                                <td>Assignment 2<br>Total</td>
                                                <td>${Math.round(i.weight * 1000) / 10}%<br>Total:A1+A2=${(Math.round(i.weight * 1000) / 10)+(Math.round(i.weight * 1000) / 10)}%</td>

                                            </tr>
                                        </c:if>

                                    </c:forEach>
                                </table>
                            </div>
                            <div>
                                <table class="bang33">
                                    <c:set var="isLastRowNeeded" value="false" />
                                    <c:forEach items="${requestScope.listGrade}" var="s">
                                        <c:if test="${s.valueScore != 0.0}">
                                            <tr class="row4">
                                                <td class="cv">${s.valueScore}</td>
                                                <td class="ck"></td>
                                            </tr>
                                        </c:if>

                                        <c:if test="${s.subjectid.subjectID == 24 or s.subjectid.subjectID == 25}">
                                            <c:set var="isLastRowNeeded" value="true" />
                                        </c:if>
                                        <c:if test="${s.subjectid.subjectID == 23}">
                                            <c:set var="isLastRowNeeded1" value="true" />
                                        </c:if>
                                    </c:forEach>

                                    <c:if test="${isLastRowNeeded}">
                                        <tr class="row4">
                                            <td></td>
                                        </tr>
                                    </c:if>
                                    <c:if test="${isLastRowNeeded1}">
                                        <tr class="row4">
                                            <td></td>
                                            <td class="ck"></td>
                                        </tr>
                                        <tr class="row4">
                                            <td></td>

                                        </tr>
                                    </c:if>
                                </table>
                            </div>
                        </div>
                        <div class="score">
                            <div class="text200">
                                COURSE TOTAL
                            </div>

                            <div class="text211">
                                AVERAGE<br/>
                                STATUS
                            </div>

                            <div class="text222">${averageScore}</div>
                            <div>
                                <c:if test="${averageScore >= 5}">
                                    <c:set var="status" value="PASS"/>
                                </c:if>
                                <c:if test="${averageScore < 5}">
                                    <c:set var="status" value="NOT PASS"/>
                                </c:if>
                                <div class="text233" style="color: ${status eq 'PASS' ? 'green' : 'red'}">${status}</div>

                            </div>

                        </div>

                        <div class="additional-line"></div>



                    </div>
                </c:if>
            </div>

            <div class="text-container hh">
                <div class="text28">
                    <h4>Mọi góp ý, thắc mắc xin liên hệ:</h4>
                </div>
                <div class="text28">
                    <h6 class="text88">Phòng dịch vụ sinh viên: Email:</h6>
                </div>
                <div class="text28">
                    <h6 class="text88"><a href="#"> dichvusinhvien@fe.edu.vn.</a></h6>
                </div>
                <div class="text28">
                    <h6 class="text88">Điện thoại:</h6>
                </div>
                <div class="text28">
                    <h6 class="text88"> (024)7308.13.13</h6>
                </div>
            </div>
            <div class="additional-line"></div>
            <div class="text33">
                <div class="circle-wrapper text33">
                    <h5>c</h5>
                </div>
                <div class="text33">
                    <h5>Powered by</h5>&nbsp;&nbsp;
                    <h5 class="text44">FPT University| CMS| library| books24x7</h5>
                </div>
            </div>

            <form/>
    </body>
</html>
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
<script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/popper.js@1.12.9/dist/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>

