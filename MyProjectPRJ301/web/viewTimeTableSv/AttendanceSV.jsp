<%-- 
    Document   : AttendanceSV
    Created on : Mar 7, 2024, 7:39:17 PM
    Author     : G5 5590
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>JSP Page</title>
        <style>
            .text1{
                margin-top: 40px;
                margin-left: -60px;

            }
            .rectangle {
                width: 1230px;
                height: 50px;
                background-color: #CCCCCC;
                margin-left: -60px;
                margin-top: 20px;
                margin-bottom: 20px;
                margin-left: 120px;
                border-radius: 5px;
            }

            table {
                width: 100%;
                border-collapse: collapse;
            }
            table, th, td {
                border: 1px solid black;
            }
            th, td {
                padding: 10px;
                text-align: left;
            }
            .cc{
                display: flex;
                justify-content: center;
            }
            .col-md-6 {
                width: 50%;
            }
            .cc{
                width: 100%;
            }
            .text2{
                margin-left: 125px;
                font-size: 30px;
            }
            .text3{
                display: flex;

                font-size: 25px;
            }
            .bang1{
                width: 600px;
                height: 50%;
                margin-left: 40px;
            }

            .text4 {
                vertical-align: top;
            }
            .text5{
                width: 500px;
            }
            .bang2{
                width: 800px;
                height: 100%
            }
            .text6{
                color: green;
            }
            .text7{
                color: red;
            }
            .text10{
                display: flex;
            }
            .text8{
                margin-left: 120px;
            }
            .text9{
                margin-left: 200px;
            }
            th{
                background-color: rgba(0, 0, 255, 0.7);
            }
            .text11{
                background-color: rgba(0, 0, 255, 0.6);
            }
            .text12{
                background-color: rgba(255, 0, 0, 0.7);
            }
            .text13{
                width: 800px;
                height: 30px;
            }
            .text28 {
                display: inline-block;
                vertical-align: top;
            }
            .text88{
                margin-top: 7px;
            }
            .hh{
                margin-top: 40px;
            }
            .additional-line {
                width: 119%;
                height: 1px;
                background-color:#000;
                margin-top: -6px;
                margin-left: -100px;
            }
            .circle-wrapper {
                width: 15px; 
                height: 15px; 
                border-radius: 50%; 
                border: 1px solid #000;
                display: flex;
                justify-content: center;
                align-items: center;
                margin-top: 5px;
            }
            .text33{
                display: flex;
                justify-content: center;
            }
            .text44{
                color: blue;
            }
             .hoho{
                background-color:  rgba(0, 128, 0, 0.6);
                border: none;
                color: white;
                margin-top: 10px;
            }
            .haha{
                 background-color:  rgba(0, 128, 0, 0.6);
                border: none;
                color: white;
                margin-top: 10px;
                height: 35px;
                width: 57px;
            }
            .hehe{

                display: flex;
                justify-content: right;

            }
            .ngan{
                margin-top: 9px;
            }
        </style>
        <script>
            function sendSemeterId(semeterId) {
                var form = document.createElement("form");
                form.setAttribute("method", "GET");
                form.setAttribute("action", "attendanceSV");

                var hiddenField = document.createElement("input");
                hiddenField.setAttribute("type", "hidden");
                hiddenField.setAttribute("name", "semeterId");
                hiddenField.setAttribute("value", semeterId);

                form.appendChild(hiddenField);
                document.body.appendChild(form);
                form.submit();
            }
            function sendSemeterId1A(semeterId1ff) {
                var form = document.createElement("form");
                form.setAttribute("method", "GET");
                form.setAttribute("action", "attendanceSV");

                var hiddenField = document.createElement("input");
                hiddenField.setAttribute("type", "hidden");
                hiddenField.setAttribute("name", "semeterId1ff");
                hiddenField.setAttribute("value", semeterId1ff);

                form.appendChild(hiddenField);
                document.body.appendChild(form);
                form.submit();
            }
        </script>

    </head>
    <body>
        <form action="attendanceSV" method="GET">
            <div class="container">
                <div class="row cc">
                    <div class="col-md-6">
                        <h1 class="text1">FPT University Academic Portal</h1>
                    </div>
                    <div class="col-md-6">
                        <img src="data:image/jpeg;base64,">

                    </div>
                </div>
            </div>  
            <div class="rectangle">
                <div class="hehe">
                    <a href="login"><input class="haha" value="Logout"/></a><h4 class="ngan">|</h4>
                    <input class="hoho" type="button" value="CAMPUS: FPTU-Hòa Lạc" readonly/>
                </div>
            </div>

            <div class="text2">View attendance for Phan Đình Tuấn (tuanpdhe171507)</div>

            <div class="text3">
                <div class="text8">
                    Select a campus/program, term, course ...
                </div>
                <div class="text9">
                    ... then see report
                </div>
            </div>
            <div class="text10"> 
                <table class="bang1">
                    <tr>
                        <th>CAMPUS/PROGRAM</th>
                        <th>TERM</th>
                        <th class="text5">COURSE</th>
                    </tr>

                    <c:forEach items="${requestScope.listSemeter}" var="i" varStatus="loop">
                        <tr>
                            <c:if test="${loop.index == 0}">
                                <td rowspan="${fn:length(requestScope.listSemeter)}">FU-HL</td>
                            </c:if>
                            <td>
                                <form id="semeterForm_${i.semeterID}" action="attendanceSV" method="POST">
                                    <div name="semeterid" onclick="document.getElementById('semeterForm_${i.semeterID}').submit()">
                                        <a href="#" onclick="sendSemeterId('${i.semeterID}')">${i.semeterName}</a>
                                    </div>
                                    <input type="hidden" name="semeterId" value="${i.semeterID}">
                                </form>
                            </td>
                            <c:if test="${loop.index == 0}">
                                <c:set var="groupNameRowCount" value="0" />
                                <c:forEach items="${requestScope.listSubject}" var="s">
                                    <c:set var="groupNameRowCount" value="${groupNameRowCount + 2}" />
                                </c:forEach>
                                <td class="text4" rowspan="${groupNameRowCount}">
                                    <c:forEach items="${requestScope.listSubject}" var="s" varStatus="status">
                                        <form id="semeterForm1ff_${s.subject.subjectID}" action="attendanceSV" method="GET">
                                            <div name="semeterid1ff" onclick="document.getElementById('semeterForm1ff_${s.subject.subjectID}').submit()">
                                                <a href="#" onclick="sendSemeterId1A('${s.subject.subjectID}')">${s.subject.subjectName}(${s.subject.code})</a>(${s.groupName}start ${s.semeter.startDate})
                                            </div>
                                            <input type="hidden" name="semeterId1ff" value="${s.subject.subjectID}">
                                            <!--                                         Thêm một hidden input để chứa semeterId 
                                                                                    <input type="hidden" name="semeterId" value="${i.semeterID}">-->
                                        </form>
                                        <br/>
                                        <c:if test="${not status.last}">
                                            <br/>
                                        </c:if>
                                    </c:forEach>
                                </td>
                            </c:if>
                        </tr>
                    </c:forEach>
                </table>




                <table class="bang2">
                    <tr>
                        <th>NO</th>
                        <th>DATE</th>
                        <th>SLOT</th>
                        <th>ROOM</th>
                        <th>LECTURER</th>
                        <th>GROUP NAME</th>
                        <th>ATTEDANCE STATUS</th>
                        <th>LECTURER'S COMMENT</th>
                    </tr>
                    <c:forEach items="${requestScope.diemDanh}" var="k" varStatus="loop">
                        <c:set var="dayOfWeek">
                            <fmt:formatDate value="${k.date}" pattern="EEEE" />
                        </c:set>



                        <tr>
                            <td>${loop.index + 1}</td>
                            <td>
                                <div class="text11">${dayOfWeek}${k.date}</div>
                            </td>
                            <td>
                                <div class="text12">${k.timeslot.name} _${k.timeslot.timeStart}${k.timeslot.timeEnd}</div>
                            </td>
                            <td>${k.room.name}</td>
                            <td>${k.teacher.code}</td>
                            <td>${k.group.groupName}</td>
                            <td>
                                <c:set var="sessionDate" value="${k.date}" />
                                <c:set var="dayNow" value="${daynow}" />
                                <c:if test="${(sessionDate.time lt dayNow.time ) and k.isTaken}">
                                    <h6 class="text6">Present</h6>
                                </c:if>

                                <c:if test="${(sessionDate.time lt dayNow.time ) and !k.isTaken}">
                                    <h6 class="text7">Absent</h6>
                                </c:if>

                                <c:if test="${(sessionDate.time gt dayNow.time ) and !k.isTaken}">
                                    <h6 class="">Future</h6>
                                </c:if>


                                <c:if test="${(sessionDate.time eq dayNow.time ) and k.isTaken}">
                                    <h6 class="text6">Present</h6>
                                </c:if>

                                <c:if test="${(sessionDate.time eq dayNow.time ) and !k.isTaken}">
                                    <h6 class="text7">absent</h6>
                                </c:if>

                                <c:if test="${sessionDate.time eq dayNow.time and (k.isTaken ne false and k.isTaken ne true)}">
                                    <h6 class="">Future</h6>
                                </c:if>
                            </td>
                            <td></td>
                        </tr>


                    </c:forEach>
                    <tr>
                        <c:set var="absentCount" value="0" />
                        <c:set var="absentpersent" value="0" />
                        <c:forEach items="${requestScope.diemDanh}" var="p" varStatus="loop">
                            <c:set var="sessionDate" value="${p.date}" />
                            <c:set var="dayNow" value="${daynow}" />
                            <%-- Nếu k.isTaken == false, tăng absentCount lên 1 --%>
                            <c:if test="${!p.isTaken and sessionDate.time le dayNow.time}">
                                <c:set var="absentCount" value="${absentCount +1}" />
                                <c:set var="absentpersent" value="${absentCount*6}" />
                            </c:if>
                        </c:forEach>

                        <%-- Hiển thị thông tin về vắng mặt --%>
                        <td colspan="8" class="text13">
                            ABSENT: ${absentpersent}% ABSENT SO FAR (${absentCount} ABSENT ON 20 TOTAL).
                        </td>
                    </tr>
                </table>
            </div>

            <div class="text-container hh">
                <div class="text28">
                    <h4>Mọi góp ý, thắc mắc xin liên hệ:</h4>
                </div>
                <div class="text28">
                    <h6 class="text88">Phòng dịch vụ sinh viên: Email:</h6>
                </div>
                <div class="text28">
                    <h6 class="text88"><a href="#"> dichvusinhvien@fe.edu.vn.</a></h6>
                </div>
                <div class="text28">
                    <h6 class="text88">Điện thoại:</h6>
                </div>
                <div class="text28">
                    <h6 class="text88"> (024)7308.13.13</h6>
                </div>
            </div>
            <div class="additional-line"></div>
            <div class="text33">
                <div class="circle-wrapper text33">
                    <h5>c</h5>
                </div>
                <div class="text33">
                    <h5>Powered by</h5>&nbsp;&nbsp;
                    <h5 class="text44">FPT University| CMS| library| books24x7</h5>
                </div>
            </div>

            <form/>
    </body>
</html>
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
<script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/popper.js@1.12.9/dist/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>

