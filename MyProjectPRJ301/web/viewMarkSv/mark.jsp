<%-- 
    Document   : AttendanceSV
    Created on : Mar 7, 2024, 7:39:17 PM
    Author     : G5 5590
--%>
...
<html>
    <head>
        ...
        <style>
            ...
            .circle-wrapper {
                ...
                height: 15px; /* Đường kính của hình tròn */
                border-radius: 50%; /* Khoanh tròn phần tử */
            ...
            .bang11 {
                width: 500px; /* Đặt độ rộng của bảng */
                table-layout: fixed; /* Thiết lập bảng để không tự động điều chỉnh kích thước cột */
            }

            .bang11 td {
                padding: 0px; /* Khoảng cách giữa nội dung và biên của các ô */
                border: 1px solid black; /* Đường viền của ô */
                text-align: left; /* Canh lề văn bản */
                word-wrap: break-word; /* Tự động ngắt dòng khi cần thiết */
            }

            /* Độ rộng cố định cho các cột */
            .bang11 td:nth-child(1), /* Cột 1 */
            .bang11 td:nth-child(2), /* Cột 2 */
            .bang11 td:nth-child(3) { /* Cột 3 */
                width: 150px; /* Độ rộng cố định cho mỗi cột */
            }
             .hoho{
        ...
        <script>
            function sendSemeterId(semeterId) {
                ...
                form.setAttribute("action", "attendanceSV");

                var hiddenField = document.createElement("input");
                hiddenField.setAttribute("type", "hidden");
                hiddenField.setAttribute("name", "semeterId");
                hiddenField.setAttribute("value", semeterId);

                form.appendChild(hiddenField);
                document.body.appendChild(form);
                form.submit();
            }
            function sendSemeterId1A(semeterId1ff) {
                var form = document.createElement("form");
                form.setAttribute("method", "GET");
                form.setAttribute("action", "attendanceSV");
                ...
                var hiddenField = document.createElement("input");
                hiddenField.setAttribute("type", "hidden");
                hiddenField.setAttribute("name", "semeterId1ff");
                hiddenField.setAttribute("value", semeterId1ff);
        ...
        <script>
            $(document).ready(function () {
                ...
                rows.each(function () {
                    var rowContent = $(this).text().trim();


                    if (seenRows[rowContent]) {
                        $(this).remove();
                    } else {
                        seenRows[rowContent] = true;
                    }
                }
                );
            }
    ...
    <body>
        <form action="marksv" method="GET">
            ...
            <div class="text2">View Mark for Phan Đình Tuấn (tuanpdhe171507)</div>
            ...
            <div class="text10"> 
                <table class="bang1">
                    <tr>
                        ...
                        <th>TERM</th>
                        <th class="text5">COURSE</th>
                    </tr>

                    <c:forEach items="${requestScope.listSemeter}" var="i" varStatus="loop">
                        <tr>
                            <c:if test="${loop.index == 0}">
                                <td rowspan="${fn:length(requestScope.listSemeter)}">FU-HL</td>
                            </c:if>
                            <td>
                                <form id="semeterForm_${i.semeterID}" action="marksv" method="POST">
                                    <div name="semeterid" onclick="document.getElementById('semeterForm_${i.semeterID}').submit()">
                                        <a href="#" onclick="sendSemeterId('${i.semeterID}')">${i.semeterName}</a>
                                    </div>
                                    <input type="hidden" name="semeterId" value="${i.semeterID}">
                                </form>
                            </td>
                            <c:if test="${loop.index == 0}">
                                <c:set var="groupNameRowCount" value="0" />
                                <c:forEach items="${requestScope.listSubject}" var="s">
                                    <c:set var="groupNameRowCount" value="${groupNameRowCount + 2}" />
                                </c:forEach>
                                <td class="text4" rowspan="${groupNameRowCount}">
                                    <c:forEach items="${requestScope.listSubject}" var="s" varStatus="status">
                                        <form id="semeterForm1ff_${s.subject.subjectID}" action="marksv" method="GET">
                                            <div name="semeterid1ff" onclick="document.getElementById('semeterForm1ff_${s.subject.subjectID}').submit()">
                                                <a href="#" onclick="sendSemeterId1A('${s.subject.subjectID}')">${s.subject.subjectName}(${s.subject.code})</a>(${s.groupName}start ${s.semeter.startDate})
                                            </div>
                                            <input type="hidden" name="semeterId1ff" value="${s.subject.subjectID}">
                                            <!--                                         Thêm một hidden input để chứa semeterId 
                                                                                    <input type="hidden" name="semeterId" value="${i.semeterID}">-->
                                        </form>
                                        <br/>
                ...
                <c:if test="${listGrade!=null && listGrade.size()>0}">
                    ...
                    <div>
                        <table class="bang">
                            ...
                            <tr>
                                <th class="text55 tuan">GRADE CATEGORY</th>
                                <th class="text66 tuan">GRADE ITEM</th>
                                <th class="text1000 tuan">WEIGHT</th>
                                <th class="text101 tuan">VALUE</th>
                                <th class="text111 tuan">COMMENT</th>
                            </tr>
                        ...
                        <div class="bangbig">
                                    ...
                                    <c:forEach items="${requestScope.listGrade}" var="i">
                                        <tr class="row2">
                                            ...
                                            <c:choose>
                                                ...
                                                <c:otherwise>
                                                    <td class="text777 kk">${i.assessmentName}<br>Total</td>
                                                    </c:otherwise>
                                                </c:choose>
                                            <td class="text999 kk">${Math.round(i.weight * 1000) / 10}%<br>Total:${Math.round(i.weight * 1000) / 10}</td>
                                        ...
                                        <c:if test="${i.assessmentName eq 'Workshop'}">
                                            <tr class="row2">
                                                <td class="kk"></td>
                                                <td class="kk">Workshop2<br>Total</td>
                                                <td class="kk">${Math.round(i.weight * 1000) / 10}%<br>Total:W1+W2=${Math.round(i.weight * 1000) / 10+Math.round(i.weight * 1000) / 10}</td>

                                            </tr>
                                        </c:if>
                                        <c:if test="${i.assessmentName eq 'Exercise'}">
                                            <tr class="row2">
                                                <td class="kk"></td>
                                                <td class="kk">Exercise2<br>Total</td>
                                                <td class="kk">${Math.round(i.weight * 1000) / 10}%<br>Total:Ex1+Ex2=${(Math.round(i.weight * 1000) / 10)+(Math.round(i.weight * 1000) / 10)}%</td>
                                        ...
                                        <c:if test="${i.assessmentName eq 'Progress test' and i.subjectid.subjectID == 25}">
                                            ...
                                            <tr class="row3 row2 text444">
                                                <td class="kk"></td>

                                                <td class="">Progress test 2<br>Total</td>
                                                <td class="kk">${Math.round(i.weight * 1000) / 10}%<br>Total:P1+P2=${Math.round(i.weight * 1000) / 10+Math.round(i.weight * 1000) / 10}%</td>
                                        ...
                                        <c:if test="${i.assessmentName eq 'Progress test' and i.subjectid.subjectID == 23}">
                                            <tr class="row2">
                                                <td class="kk"></td>
                                                <td class="kk">Progress test 2<br>Total</td>
                                                <td class="kk">${Math.round(i.weight * 1000) / 10}%<br>Total:P1+P2=${(Math.round(i.weight * 1000) / 10)+(Math.round(i.weight * 1000) / 10)}%</td>
                                        ...
                                        <c:if test="${i.assessmentName eq 'Progress test' and i.subjectid.subjectID == 22}">
                                            ...
                                            <tr class="row3 row2 text444">
                                                ...
                                                <td class="kk">Progress test 2</td>
                                                <td class="kk">${Math.round(i.weight * 1000) / 10}%<br>Total:${Math.round(i.weight * 1000) / 10}%</td>
                                        ...
                                        <c:if test="${i.assessmentName eq 'Progress test' and i.subjectid.subjectID == 22}">
                                            ...
                                            <tr class="row3 row2 text444">
                                                ...
                                                <td class="kk">Progress test 3<br>Total</td>
                                                <td class="kk">${Math.round(i.weight * 1000) / 10}%<br>Total:P1+P2+P3=${(Math.round(i.weight * 1000) / 10)+(Math.round(i.weight * 1000) / 10)+(Math.round(i.weight * 1000) / 10)}%</td>

                                            </tr>
                                        </c:if>
                                        <c:if test="${i.assessmentName eq 'Assignment' and i.subjectid.subjectID == 22}">
                                            <tr class="row2">
                                                <td class="kk"></td>
                                                <td class="kk">Assignment 2<br>Total</td>
                                                <td class="kk">${Math.round(i.weight * 1000) / 10}%<br>Total:A1+A2=${(Math.round(i.weight * 1000) / 10)+(Math.round(i.weight * 1000) / 10)}%</td>
                            ...
                            <div>
                                <table class="bang33">
                                    <c:set var="isLastRowNeeded" value="false" />
                                    <c:forEach items="${requestScope.listGrade}" var="s">
                                        <c:if test="${s.valueScore != 0.0}">
                                            <tr class="row4">
                                                <td class="cv">${s.valueScore}</td>
                                                <td class="ck"></td>
                                            </tr>
                                        </c:if>
                        ...
                        <div class="score">
                            ...
                            <div class="text222">${averageScore}</div>
                            <div>
                                ...
                                <div class="text233" style="color: ${status eq 'PASS' ? 'green' : 'red'}">${status}</div>
            ...
            <div class="text-container hh">
                <div class="text28">
                    <h4>Mọi góp ý, thắc mắc xin liên hệ:</h4>
                </div>
                <div class="text28">
                    <h6 class="text88">Phòng dịch vụ sinh viên: Email:</h6>
                </div>
                <div class="text28">
                    <h6 class="text88"><a href="#"> dichvusinhvien@fe.edu.vn.</a></h6>
                </div>
                <div class="text28">
                    <h6 class="text88">Điện thoại:</h6>
                </div>
                <div class="text28">
                    <h6 class="text88"> (024)7308.13.13</h6>
                </div>
            </div>
            <div class="additional-line"></div>
            <div class="text33">
                <div class="circle-wrapper text33">
                    <h5>c</h5>
                </div>
                <div class="text33">
                    <h5>Powered by</h5>&nbsp;&nbsp;
                    <h5 class="text44">FPT University| CMS| library| books24x7</h5>
                </div>
            </div>
